...
struct ExprResult *
MakeExprResult(enum BaseTypes returnType) {
...
void
FreeExprResult(struct ExprResult * expr) {
  if( expr->instr ) free( expr->instr ); //FreeSeq( expr->instr );
  free( expr );
}

void
FreeCondResult(struct CondResult * cond) {
  if( cond->instr ) free( cond->instr ); // FreeSeq( cond->instr );
  free( cond->label );
  free( cond );
}
